...
#   python -m PyInstaller --noconsole --OffboardV15.py
...
btnOpenSelected = ttk.Button(btns_frame, text="Pop-out Selected Users")
btnOpenSelected.pack(side=tk.LEFT, padx=(0,8))
...
selected_window = {"wnd": None, "tree": None}  # for pop-out window
...
# ---------- Pop-out Selected Users Window -------------
def open_selected_window():
    if selected_window["wnd"] and tk.Toplevel.winfo_exists(selected_window["wnd"]):
        selected_window["wnd"].lift()
        refresh_selected_window()
        return

    wnd = tk.Toplevel(root)
    wnd.title("Selected Users (Persistent)")
    wnd.geometry("950x420")
    wnd.transient(root)

    sel_tree = ttk.Treeview(wnd, columns=sel_cols, show="headings", selectmode="extended")
    for col, w in zip(sel_cols, (220,160,260,520)):
        sel_tree.heading(col, text=col)
        sel_tree.column(col, width=w, anchor="w", stretch=True if col=="DistinguishedName" else False)
    ysb = ttk.Scrollbar(wnd, orient="vertical", command=sel_tree.yview)
    sel_tree.configure(yscroll=ysb.set)
    sel_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(8,0), pady=8)
    ysb.pack(side=tk.LEFT, fill=tk.Y, pady=8)

    btns = ttk.Frame(wnd)
    btns.pack(side=tk.RIGHT, fill=tk.Y, padx=8, pady=8)

    def remove_selected_from_bucket():
        rows = sel_tree.selection()
        if not rows: return
        removed = 0
        for iid in rows:
            dn = sel_tree.item(iid, "values")[3]
            if dn in selection_bucket:
                del selection_bucket[dn]; removed += 1
            if dn in selected_dns:
                selected_dns.remove(dn)
        refresh_selected_views()
        update_selected_count()
        if removed: log(f"Removed {removed} from Selected bucket.", "OK")
    ...
    def offboard_from_window():
        bucket_users = list(selection_bucket.values())
        if not bucket_users:
            messagebox.showinfo("Offboarding", "No users in Selected bucket."); return
        names = ", ".join([u.get("Name","") for u in bucket_users][:5])
        more = "" if len(bucket_users) <= 5 else f" (+{len(bucket_users)-5} more)"
        if messagebox.askyesno("Confirm", f'Proceed with offboarding for {len(bucket_users)} user(s): {names}{more}?') != True:
            return
    ...
    ttk.Button(btns, text="Remove Selected", command=remove_selected_from_bucket).pack(fill=tk.X, pady=(0,8))
    ttk.Button(btns, text="Clear All", command=clear_bucket).pack(fill=tk.X, pady=(0,8))
    ttk.Button(btns, text="Offboard All", command=offboard_from_window).pack(fill=tk.X)

    selected_window["wnd"] = wnd
    selected_window["tree"] = sel_tree
    refresh_selected_window()

def refresh_selected_window():
    if not (selected_window["wnd"] and tk.Toplevel.winfo_exists(selected_window["wnd"])):
        return
    sel_tree = selected_window["tree"]
    sel_tree.delete(*sel_tree.get_children())
    for dn, u in selection_bucket.items():
        sel_tree.insert("", "end", values=(
            u.get("Name",""),
            u.get("SamAccountName",""),
            u.get("EmailAddress",""),
        ...
        if sel:
            ...
            if idx is not None:
                ...
                if u.get("DistinguishedName",""):
                    bucket_users = [u]
    if not bucket_users:
        messagebox.showinfo("Offboarding", "Select (check) users, or use the Selected Users panel/window."); return
...
btnOpenSelected.configure(command=open_selected_window)
...
set_status("Ready. Search by email or name (comma-separated). Check users to build your batch; use the Selected Users panel/window; then Offboard Selected.")
root.mainloop()
